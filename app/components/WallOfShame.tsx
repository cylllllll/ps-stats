...
export default function WallOfShame({ games }: { games: PlayStationGame[] }) {
  ...
  return <div className="space-y-6"><div className="grid grid-cols-2 md:grid-cols-3 gap-4"><Stat title="未满奖杯游戏" value={String(candidates.length)} icon={<Trophy className="h-4 w-4" />} /><Stat title="0% 进度" value={String(candidates.filter((game) => game.trophyProgress === 0).length)} icon={<Gamepad2 className="h-4 w-4" />} /><Stat title="最高待完成进度" value={`${highestProgress}%`} icon={<Dices className="h-4 w-4" />} /></div><Card className="overflow-hidden border-amber-200 dark:border-amber-900/50 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30"><CardHeader className="text-center"><CardTitle className="flex items-center justify-center gap-2"><Dices className="h-5 w-5 text-amber-600 dark:text-amber-400" />抽一款继续完成</CardTitle><p className="text-sm text-muted-foreground">只使用 PSN 返回的未完成奖杯记录；同一游戏已有完成版本时不重复计入。</p></CardHeader><CardContent className="flex flex-col items-center gap-5">{randomGame ? <div className="w-full max-w-md rounded-2xl overflow-hidden bg-white dark:bg-card border shadow-sm"><div className="aspect-square flex items-center justify-center bg-muted"><img src={randomGame.iconUrl} alt={randomGame.name} className="w-full h-full object-cover" /></div><div className="p-5 text-center"><h3 className="text-xl font-bold">{randomGame.name}</h3><div className="flex justify-center items-center gap-2 mt-2"><Badge variant="secondary">{randomGame.trophyProgress}% 奖杯</Badge><PlatformBadge platform={randomGame.platform} badgeVariant="outline" /></div><p className="text-sm text-muted-foreground mt-3">已游玩 {formatDuration(randomGame.playtimeSeconds)}</p></div></div> : <p className="text-muted-foreground py-8">恭喜，所有游戏都已完成！</p>}<Button onClick={spin} disabled={!candidates.length} className="gap-2"><Dices className="h-4 w-4" />再抽一次</Button></CardContent></Card><Card><CardHeader><CardTitle className="text-base">待完成游戏</CardTitle></CardHeader><CardContent><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">{candidates.map((game) => <div key={game.id} className="group relative flex items-center justify-center"><div className="aspect-square relative w-full overflow-hidden rounded-xl bg-muted"><img src={game.iconUrl} alt={game.name} className="w-full h-full object-cover transition-transform group-hover:scale-105" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent px-2 pt-8 pb-2"><p className="text-white text-xs font-medium truncate">{game.name}</p><div className="flex flex-wrap items-center gap-1 text-white/70 text-[10px]"><PlatformBadge platform={game.platform} className="px-1 py-0 text-[9px] bg-white/20 text-white border-0" /><span>·</span><span>{game.trophyProgress}%</span><span>·</span><span>{formatDuration(game.playtimeSeconds)}</span></div></div></div></div>)}</div></CardContent></Card></div>;
}
